...
window.addEventListener("load", () => {
  ...
  function initConnectScene() {
    ...
    function setupDrone(g) {
      ...
      // Força re-warm com o drone já na cena, independente de pre-warm anterior sem drone.
      // Garante que MeshPhysicalMaterials sejam compilados em qualquer ordem de chegada (GLB vs idle).
      preWarmed = false
      preWarmScene()
    }
    ...
    scheduleIdle(() => {
      ...
      // Pre-upload das texturas canvas para a GPU durante o preloader
      renderer.initTexture(cf.albedo)
      renderer.initTexture(cf.rough)
      renderer.initTexture(cf.normal)
      renderer.initTexture(sol.albedo)
      renderer.initTexture(sol.rough)
      renderer.initTexture(sol.normal)

      // GLB may have already arrived while textures were being generated
      // setupDrone() chama preWarmScene() internamente (com drone na cena)
      if (pendingGltf) { setupDrone(pendingGltf); pendingGltf = null }
      else { preWarmScene() } // drone ainda não chegou — pre-warm só com céu e nuvens
    })
    ...
    // Pre-warm: compila todos os shaders e sobe texturas para GPU antes da seção entrar na viewport.
    // Chamado durante o preloader (scheduleIdle + setupDrone), não durante o scroll.
    let preWarmed = false
    function preWarmScene() {
      if (preWarmed) return
      preWarmed = true
      const prev = renderer.toneMapping
    ...
    new IntersectionObserver(([entry]) => {
      ...
      if (!isVisible) {
        ...
        preWarmScene() // fallback: garante pre-warm mesmo se o preloader foi rápido
