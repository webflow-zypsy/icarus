...
const ke = (r = {}) => {
    const o = r.towCount || 32, t = 512 / o, e = r.gap || 1;
...
fe.load(Ue, r => {
    ...
    const _camP = new E(), _tgtP = new E();

    const a = s => {
        const l = Math.max(0, Math.min(1, s)), C = M.length - 1, f = l * C, m = Math.min(Math.floor(f), C - 1), d = f - m;
        _camP.lerpVectors(M[m].cam, M[m + 1].cam, d);
        _tgtP.lerpVectors(M[m].tgt, M[m + 1].tgt, d);
        Y.position.copy(_camP); se.copy(_tgtP); Y.lookAt(se);
    };
    
    a(0);

    const trEl = document.getElementById("scenes-track");

    if (!ie) {
        window.addEventListener("scroll", () => {
            if (trEl) {
                const rect = trEl.getBoundingClientRect();
                const dist = trEl.offsetHeight - window.innerHeight;
...
function pe() {
    ...
    z.render(P, Y);
    peRafId = null;
    if (peVisible) peRafId = requestAnimationFrame(pe);
}
...
function startPe() {
    if (peRafId !== null) return;
    peRafId = requestAnimationFrame(pe);
}
function stopPe() {
    if (peRafId !== null) { cancelAnimationFrame(peRafId); peRafId = null; }
}

new IntersectionObserver(([entry]) => {
    peVisible = entry.isIntersecting;
    peVisible ? startPe() : stopPe();
}, { threshold: 0 }).observe(D);

document.addEventListener("visibilitychange", () => {
    if (document.hidden) { stopPe(); }
    else if (peVisible) { startPe(); }
});
