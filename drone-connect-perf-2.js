...
x.setClearColor(0, 0);
// Pre-upload das texturas canvas para a GPU imediatamente (geradas síncronamente acima)
x.initTexture(Ee.albedo); x.initTexture(Ee.rough); x.initTexture(Ee.normal);
x.initTexture(Fe.albedo); x.initTexture(Fe.rough); x.initTexture(Fe.normal);
...
const Ot = "https://cdn.jsdelivr.net/gh/webflow-zypsy/icarus@main/green-512.hdr";
const _t = "https://cdn.jsdelivr.net/gh/webflow-zypsy/icarus@main/apollo-draco.glb";
const Wt = "https://cdn.jsdelivr.net/gh/webflow-zypsy/icarus@main/vienna-mountains.webp";
const Ht = "https://cdn.jsdelivr.net/gh/webflow-zypsy/icarus@main/night.webp";
const cloudUrl = "https://cdn.jsdelivr.net/gh/webflow-zypsy/icarus@main/cloud03-7.webp";
...
A = i;
// Re-warm com drone na cena — garante compilação dos MeshPhysicalMaterials
_preWarmed = false;
preWarmScene();
});
...
const earlyEnd = 0.5 * window.innerHeight;
...
if (c) { e = Math.max(-Math.PI / 2 + 0.01, Math.min(Math.PI / 2 - 0.01, e)); t(); N.uniforms.uHFov.value = r; }
});
}
...
x.autoClear = true;
ltRafId = null;
if (ltVisible) ltRafId = requestAnimationFrame(lt);
}

// ---------- Cache DOM references used every frame ----------
const nightTintEl = document.getElementById("night-tint");

// ---------- RAF loop: starts immediately, pauses when off-screen or tab hidden ----------
let ltRafId = null;
let ltVisible = true;
...
function startLt() {
  if (ltRafId !== null) return;
  preWarmScene(); // fallback: garante pre-warm antes do primeiro frame
  ltRafId = requestAnimationFrame(lt);
}
function stopLt() {
  if (ltRafId !== null) { cancelAnimationFrame(ltRafId); ltRafId = null; }
}

// Start immediately — assets load in the background regardless
startLt();

// Pause/resume as the section enters and leaves the viewport
if (Y) {
  new IntersectionObserver(([entry]) => {
    ltVisible = entry.isIntersecting;
    ltVisible ? startLt() : stopLt();
  }, { threshold: 0 }).observe(Y);
}

document.addEventListener("visibilitychange", () => {
  if (document.hidden) { stopLt(); }
  else if (ltVisible) { startLt(); }
});
