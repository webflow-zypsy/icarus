 ...
 *
 * Camera poses, drone offset, cloud parallax and env rotation are taken
 * directly from drone-about-v6.js. That scene uses extraScale:16 × droneScale:0.5
 * making the effective model 8× larger than ours (1.4 units). All world-space
 * positions are therefore divided by 8 to match our coordinate space.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
window.addEventListener("load", () => {
  ...
  // ── Bob config — from drone-about-v6, bobAmp scaled ÷8 ───────────────────
  const bobCfg = { bobAmp: 0.005, bobPeriod: 5.0, stallPeriod: 3.0, stallDepth: 0.35, pitchAmp: 0.0075 }

  // ── Drone offset — v6 pos1=(0.50,0,-3.50) pos2=(-2.50,0,-4.50) ÷8 ───────
    ...
    mat.onBeforeCompile = s => {
      ...
      s.fragmentShader = s.fragmentShader
        ...
        .replace("vec4 diffuseColor = vec4( diffuse, opacity );",
          "vec4 diffuseColor=vec4(diffuse,opacity);\n{float revDist=max(abs(vRevealWorldPos.x),abs(vRevealWorldPos.z));if(revDist>revealRadius)discard;}\n")
    }
  ...
  function makeCarbonFiberTextures(opts = {}) {
    ...
    for(let row=0;row<tc;row++) for(let col=0;col<tc;col++){
      const x=col*tp,y=row*tp,isWarpOver=((col+row)%4)<2
      ...
      for(let s=0;s<5;s++){
        ...
        if(isWarpOver){const sx=x+gap+((tp-gap*2)*(s+0.5))/5;a.moveTo(sx,y+gap);a.lineTo(sx,y+tp-gap)}
    ...
    for(let row=0;row<tc;row++) for(let col=0;col<tc;col++){
      const x=col*tp,y=row*tp,isWarpOver=((col+row)%4)<2
      const v=isWarpOver?br-6+Math.random()*4:br+2+Math.random()*6
  ...
  function makeSolarPanelTextures(opts={}) {
    const sz=512,cellCols=opts.cellCols||4,cellRows=opts.cellRows||6
    const cellGap=opts.cellGap||10,busBarCount=opts.busBarCount||5,fingerSpacing=opts.fingerSpacing||4
    const cellW=(sz-(cellCols+1)*cellGap)/cellCols,cellH=(sz-(cellRows+1)*cellGap)/cellRows
    const cellX=col=>cellGap+col*(cellW+cellGap),cellY=row=>cellGap+row*(cellH+cellGap)
    const mk=()=>{const c=document.createElement("canvas");c.width=c.height=sz;return{c,ctx:c.getContext("2d")}}
    const{c:aC,ctx:a}=mk()
    a.fillStyle="#474751"; a.fillRect(0,0,sz,sz)
    for(let row=0;row<cellRows;row++) for(let col=0;col<cellCols;col++){
      const x=cellX(col),y=cellY(row),rv=Math.random()*4-2
      ...
      for(let b=0;b<busBarCount;b++){
        ...
        a.strokeStyle="rgba(50,50,58,0.95)";a.lineWidth=1.5;a.beginPath();a.moveTo(x,barY);a.lineTo(x+cellW,barY);a.stroke()
      }
      ...
      for(let b=0;b<busBarCount;b++){
        ...
        for(let fx=x+fingerSpacing*3;fx<x+cellW;fx+=fingerSpacing*4){
          a.save();a.translate(fx,barY);a.rotate(Math.PI/4);a.fillStyle="rgba(55,55,65,0.7)";a.fillRect(-1.5,-1.5,3,3);a.restore()
        }
      }
    }
    const{c:rC,ctx:r}=mk()
    ...
    for(let row=0;row<cellRows;row++) for(let col=0;col<cellCols;col++){
      const x=cellX(col),y=cellY(row),cv=50+Math.random()*5
      ...
      for(let b=0;b<busBarCount;b++){
        const barY=y+cellH*(b+1)/(busBarCount+1)
        r.strokeStyle="rgb(30,30,30)";r.lineWidth=1.5;r.beginPath();r.moveTo(x,barY);r.lineTo(x+cellW,barY);r.stroke()
      }
    }
    const{c:nC,ctx:n}=mk()
    n.fillStyle="rgb(128,128,255)";n.fillRect(0,0,sz,sz)
  ...
  // v6 pose 0: cam(19.28,16.29,25.20) ÷8 = (2.410,2.036,3.150)
  // v6 pose 1: cam(24.92,12.85,22.04) ÷8 = (3.115,1.606,2.755)
  // v6 tgt: (0.6,0.98,0) ÷8 = (0.075,0.123,0)
  const poses = [
  ...
  const _camTarget = new THREE.Vector3(poses[0].tgt.x, poses[0].tgt.y, poses[0].tgt.z)

  // Initialise camera at pose 0 immediately
  ...
  function applyPose(t) {
    ...
    camera.lookAt(_camTarget)
    // FOV: 15 → 12.75 over full scroll (matches v6's 15% zoom)
  ...
  // ── Cloud parallax ────────────────────────────────────────────────────────
  // v6: cloud03-7 at (-12,-4,-8) scale 28×14, CLOUD_PARALLAX=0.7
  ...
  const cloudCamStart = poses[0].cam.clone()
  ...
  function createCloudPlane(url, x, y, z, sx, sy, opacity) {
  ...
  // cloud03-7: bottom-left (v6: -12,-4,-8 ÷8)
  createCloudPlane(CONNECT_DRONE_ASSETS.cloud1, -1.5, -0.5, -1.0, 3.5, 1.75)
  // cloud03-8: upper-right, slightly behind, adds depth
  createCloudPlane(CONNECT_DRONE_ASSETS.cloud2,  1.2, -0.3, -0.8, 2.8, 1.4, 0.7)
  ...
  function animate(){
    ...
    // Drone position: base + scroll-lerped offset + bob
    if(droneObject){
