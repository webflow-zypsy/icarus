...
// =============================================================================
// Scroll — GSAP ScrollTrigger scrubbing #scenes-track
// =============================================================================

// Proxy object that GSAP writes to; the render loop reads it each frame.
// Using a single shared `t` means both cameras always read the same scrubbed value —
// no manual lerp needed, ScrollTrigger's scrub:1 handles the smoothing.
const scrollProxy = { t: 0 }

// ScrollTrigger is set up after the DOM is ready. If the GLB hasn't loaded yet
// the pose appliers simply won't exist, which is fine — the animate loop guards them.
ScrollTrigger.create({
  trigger: "#scenes-track",       // the 400vh parent div
  start:   "top top",             // when top of track hits top of viewport
  end:     "bottom bottom",       // when bottom of track hits bottom of viewport
  scrub:   1,                     // 1s lag — cinematic smoothing
  onUpdate: (self) => {
    scrollProxy.t = self.progress
  },
})

window.addEventListener("message", (e) => {
  if (e.data && typeof e.data.scrollProgress === "number") scrollProxy.t = e.data.scrollProgress
...
function animate(now) {
  ...
  // --- Scroll-driven camera poses (GSAP scrub handles smoothing) ---
  if (window.__applyDronePose) window.__applyDronePose(scrollProxy.t)
  if (window.__applyBgPose)    window.__applyBgPose(scrollProxy.t)
  ...
  if (bgAnim.phase === "done") {
    const scrollPct  = Math.min(scrollProxy.t / 0.5, 1)
