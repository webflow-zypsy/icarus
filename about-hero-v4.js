...
;(function () {
  ...
  const cfMatte  = makeCarbonFiberTextures({ towCount: 24 })
  ...
  function animate() {
    ...
    if (droneObject) {
      ...
      const camFwd   = _camFwd.subVectors(droneBasePos, camera.position).normalize()
      const camRight = _camRight.crossVectors(_worldUp, camFwd).normalize()
      const camUp    = _camUp.crossVectors(camFwd, camRight).normalize()
